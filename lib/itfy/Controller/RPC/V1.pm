...
sub default :Path :Args
{
  ...
  if (!defined $machine)
  {
    ...
    $c->response->body(JSON::Any->objToJson( { error => "API Key is incorrect" } ) );
...
sub get_rev_bydate
{
  ...
  my $rev = qx#git log --first-parent --before="$date" --format=%H origin/master | head -1#;
  chomp $rev;
...
sub add_branch_rev :Private
{
  ...
  eval
  {
    my $new_rev = $branch->create_related("revs", {
